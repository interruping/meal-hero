...
PRD §7 "에셋 제작 요구"가 기준. 도구 배정:

- **Meshy 3D 생성 (필수)**: 주인공 캐릭터, 방해요소 4종(전단지 알바생·자전거 초딩·비둘기·취객), 탈것 4종 — 고유 실루엣이 필요한 것들. 무료 에셋으로 대체 가능하면 그쪽 우선
- **gpt-image-2 텍스처 (필수)**: 건물 20종+ 외벽/간판, 장식 오브젝트, 지면·도로·하늘 계절 텍스처. 단순 지오메트리에 AI 텍스처를 입히는 방식이 기본 (Meshy 크레딧 절약)
- **단색 프리미티브로 최종 씬 구성 금지.** 색만 칠한 박스는 블록아웃(개발 중간 단계)에서만 허용. "로우폴리 스타일"을 텍스처 생략의 근거로 쓰지 말 것
- 블록아웃 → 에셋 교체 순서로 작업하되, **에셋 교체까지 끝나야 해당 FR 완료**로 판정
